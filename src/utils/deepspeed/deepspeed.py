...
class DeepspeedStrategy(ABC):
    ...
    def _ds_init_train_model(self, model, optim, scheduler):
        ...
        ds_config = self.get_ds_train_config(is_wrapper)

        if self.ds_tensor_parallel_size > 1:
            tp_model = deepspeed.tp_model_init(
                model=self._extract_model(model), tp_size=self.ds_tensor_parallel_size, dtype=torch.bfloat16
            )
            if is_wrapper:
    ...
    def save_model(self, model: nn.Module, tokenizer, output_dir, **kwargs) -> None:
        ...
        if self.is_rank_0():
            ...
            if isinstance(model_to_save, PeftModel):
                ...
                if (self.ds_tensor_parallel_size > 1 or self.stage == 3) and get_peft_model_state_dict is not None:
                    torch.save(
